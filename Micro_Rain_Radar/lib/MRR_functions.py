...
# convert raw files in MRR Doppler moments
def raw2snow(file_in, file_out, TRES=60, Descr="MRR data", author="APRES3 project", ncForm="NETCDF3_CLASSIC", removeOverlap=False, varsToSave="all"):
    import os
    import time
    import warnings
    import sys
    ...
    rawData = IMProToo.mrrRawData(
        file_in, removeOverlap=removeOverlap)  # Read RawData
    # create the IMProToo object and load rawData
    processedSpec = IMProToo.MrrZe(rawData)
    # integrates the data in 60 seconds by default.
    processedSpec.averageSpectra(TRES)
    ...
    # Saves the processed data in a nc file
    processedSpec.writeNetCDF(file_out, ncForm=ncForm, varsToSave=varsToSave)


def MRRQlooks(file_in, file_out, Ze_ranges=[-10, 60], W_ranges=[-1, 10], SW_ranges=[0, 3], S_ranges=[0, 0.4], format='png', dpi=300, name_station='', max_h=3, height_plot=False, cmap='jet', Zecorrected=True, IncludeS=True):
    import numpy as np
    ...
    if IncludeS == True:
        S = ds.variables["SnowfallRate"][:]  # .T #S mm/h
    ...
    H = np.ma.array(H)
    NoDataMask = np.ma.masked_where(
        H.filled(-1) > 0, np.ones(shape=np.shape(Ze)))
    N_NoData = np.sum(H[:, 0].filled(-1) == -1)
    #print('N profiles without Data = ',N_NoData)

    # Get year, month and day
    # using the center of the file as referece
    time_structure = time.gmtime(np.ma.mean(Time))
    ...
    time_ticks = [str(time.gmtime(j).tm_hour).zfill(2)
                  for j in time_ticks_epoch]
    ...
    for i in range(1, n+1):
        ...
        if (i == 1) & (N_NoData > 0):
            plt.plot([-1, 1], [-1, 1], '-', color='black',
                     label='No Data', linewidth=2)
            legend = plt.legend(loc='upper right')
            legend.get_frame().set_edgecolor('1.0')

        if i == 1:
            vmMn = [Ze_ranges[0], Ze_ranges[1], r'$Z_e$']
        if i == 2:
            vmMn = [W_ranges[0], W_ranges[1], r'$W$']
        if i == 3:
            vmMn = [SW_ranges[0], SW_ranges[1], r'$\sigma$']
        if i == 4:
            vmMn = [S_ranges[0], S_ranges[1], 'Snowfall rate']

        plt.pcolormesh(Time, h, np.transpose(NoDataMask),
                       shading='auto', cmap='gray')
        plt.pcolormesh(Time, h, np.transpose(
            mat[i-1]), vmin=vmMn[0], vmax=vmMn[1], cmap=cmap, shading='auto')

        if i == 1:
            plt.colorbar(label=r'$Z_e$'+" [dB"+r'$Z_e$'+"]")
            plt.title(name_station+" - MRR, "+str(year).zfill(4) +
...
def MFDataset_save(fileout, ds_merged, format="NETCDF3_CLASSIC"):
    ...
    range = ds_merged2.createVariable(
        'range', 'i', ('range',), fill_value=-9999)
    velocity = ds_merged2.createVariable(
        'velocity', 'f', ('velocity'), fill_value=-9999.)
    height = ds_merged2.createVariable(
        'height', 'f', ('time', 'range'), fill_value=-9999.)
    Ze = ds_merged2.createVariable(
        'Ze', 'f', ('time', 'range'), fill_value=-9999.)
    W = ds_merged2.createVariable(
        'W', 'f', ('time', 'range'), fill_value=-9999.)
    ...
    if 'quality' in varkeys:
        quality = ds_merged2.createVariable(
            'quality', 'i', ('time', 'range',), fill_value=-9999)
    ...
    if 'etaMask' in varkeys:
        etaMask = ds_merged2.createVariable(
            'etaMask', 'i', ('time', 'range', 'velocity'), fill_value=-9999)
    ...
    if 'eta' in varkeys:
        eta = ds_merged2.createVariable(
            'eta', 'f', ('time', 'range', 'velocity'), fill_value=-9999.)
        eta[:] = ds_merged.variables['eta'][:]
        eta.description = ds_merged.variables['eta'].description
        eta.units = ds_merged.variables['eta'].units
    if 'TF' in varkeys:
        TF = ds_merged2.createVariable(
            'TF', 'f', ('time', 'range'), fill_value=-9999.)
    ...
    if 'spectralWidth' in varkeys:
        spectralWidth = ds_merged2.createVariable(
            'spectralWidth', 'f', ('time', 'range'), fill_value=-9999.)
    ...
    if 'skewness' in varkeys:
        skewness = ds_merged2.createVariable(
            'skewness', 'f', ('time', 'range'), fill_value=-9999.)
    ...
    if 'kurtosis' in varkeys:
        kurtosis = ds_merged2.createVariable(
            'kurtosis', 'f', ('time', 'range'), fill_value=-9999.)
    ...
    if 'peakVelLeftBorder' in varkeys:
        peakVelLeftBorder = ds_merged2.createVariable(
            'peakVelLeftBorder', 'f', ('time', 'range'), fill_value=-9999.)
    ...
    if 'peakVelRightBorder' in varkeys:
        peakVelRightBorder = ds_merged2.createVariable(
            'peakVelRightBorder', 'f', ('time', 'range'), fill_value=-9999.)
    ...
    if 'leftSlope' in varkeys:
        leftSlope = ds_merged2.createVariable(
            'leftSlope', 'f', ('time', 'range'), fill_value=-9999.)
    ...
    if 'rightSlope' in varkeys:
        rightSlope = ds_merged2.createVariable(
            'rightSlope', 'f', ('time', 'range'), fill_value=-9999.)
    ...
    if 'etaNoiseAve' in varkeys:
        etaNoiseAve = ds_merged2.createVariable(
            'etaNoiseAve', 'f', ('time', 'range'), fill_value=-9999.)
    ...
    if 'etaNoiseStd' in varkeys:
        etaNoiseStd = ds_merged2.createVariable(
            'etaNoiseStd', 'f', ('time', 'range'), fill_value=-9999.)
    ...
    if 'SNR' in varkeys:
        SNR = ds_merged2.createVariable(
            'SNR', 'f', ('time', 'range'), fill_value=-9999.)
        SNR[:] = ds_merged.variables['SNR'][:]
        SNR.description = ds_merged.variables['SNR'].description
        SNR.units = ds_merged.variables['SNR'].units
    if 'ZeX' in varkeys:
        ZeX = ds_merged2.createVariable(
            'ZeX', 'f', ('time', 'range'), fill_value=-9999.)
    ...
    if 'SnowfallRate' in varkeys:
        S = ds_merged2.createVariable(
            'SnowfallRate', 'f', ('time', 'range'), fill_value=-9999.)
