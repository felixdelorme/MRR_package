...
## ADDED "," errors='ignore'" in "allData = open(file, 'r', errors='ignore')""
...
class MrrZe:
    ...
    def writeNetCDF(self, fname, varsToSave="all", ncForm="NETCDF3_CLASSIC"):
        ...
        ncShape2D = ("time", "range",) #NOT MODIFIED YET
        ...
        nc_height.units = 'm'
        nc_height[:] = np.array(self.H.filled(self.missingNumber), dtype="f4")#.T
        ...
        if varsToSave == 'all' or "quality" in varsToSave:
            ...
            nc_qual.units = "bin"
            nc_qual[:] = np.array(qualArray, dtype="i4")#.T
        ...
        if varsToSave == 'all' or "TF" in varsToSave:
            ...
            nc_TF.units = "-"
            nc_TF[:] = np.array(self.TF.filled(self.missingNumber), dtype="f4")#.T
        ...
        if (varsToSave == 'all' and saveAlsoNonDealiased) or "Ze_noDA" in varsToSave:
            ...
            nc_ze_noDA[:] = np.array(self.Ze_noDA, dtype="f4")#.T
        ...
        if varsToSave == 'all' or "Ze" in varsToSave:
            ...
            nc_ze.units = "dBz"
            nc_ze[:] = np.array(self.Ze, dtype="f4")#.T
        ...
        if (varsToSave == 'all' and saveAlsoNonDealiased) or "specWidth_noDA" in varsToSave:
            ...
            nc_specWidth_noDA[:] = np.array(self.specWidth_noDA, dtype="f4")#.T
        ...
        if varsToSave == 'all' or "specWidth" in varsToSave:
            ...
            nc_specWidth[:] = np.array(self.specWidth, dtype="f4")#.T
        ...
        if (varsToSave == 'all' and saveAlsoNonDealiased) or "skewness_noDA" in varsToSave:
            ...
            nc_skewness_noDA[:] = np.array(self.skewness_noDA, dtype="f4")#.T
        ...
        if varsToSave == 'all' or "skewness" in varsToSave:
            ...
            nc_skewness[:] = np.array(self.skewness, dtype="f4")#.T
        ...
        if (varsToSave == 'all' and saveAlsoNonDealiased) or "kurtosis_noDA" in varsToSave:
            ...
            nc_kurtosis_noDA[:] = np.array(self.kurtosis_noDA, dtype="f4")#.T
        ...
        if varsToSave == 'all' or "kurtosis" in varsToSave:
            ...
            nc_kurtosis[:] = np.array(self.kurtosis, dtype="f4")#.T
        ...
        if (varsToSave == 'all' and saveAlsoNonDealiased) or "peakVelLeftBorder_noDA" in varsToSave:
            ...
            nc_peakVelLeftBorder_noDA[:] = np.array(
                self.peakVelLeftBorder_noDA, dtype="f4")#.T
        ...
        if varsToSave == 'all' or "peakVelLeftBorder" in varsToSave:
            ...
            nc_peakVelLeftBorder[:] = np.array(
                self.peakVelLeftBorder, dtype="f4")#.T
        ...
        if (varsToSave == 'all' and saveAlsoNonDealiased) or "peakVelRightBorder_noDA" in varsToSave:
            ...
            nc_peakVelRightBorder_noDA[:] = np.array(
                self.peakVelRightBorder_noDA, dtype="f4")#.T
        ...
        if varsToSave == 'all' or "peakVelRightBorder" in varsToSave:
            ...
            nc_peakVelRightBorder[:] = np.array(
                self.peakVelRightBorder, dtype="f4")#.T
        ...
        if (varsToSave == 'all' and saveAlsoNonDealiased) or "leftSlope_noDA" in varsToSave:
            ...
            nc_leftSlope_noDA[:] = np.array(self.leftSlope_noDA, dtype="f4")#.T
        ...
        if varsToSave == 'all' or "leftSlope" in varsToSave:
            ...
            nc_leftSlope[:] = np.array(self.leftSlope, dtype="f4")#.T
        ...
        if (varsToSave == 'all' and saveAlsoNonDealiased) or "rightSlope_noDA" in varsToSave:
            ...
            nc_rightSlope_noDA[:] = np.array(self.rightSlope_noDA, dtype="f4")#.T
        ...
        if varsToSave == 'all' or "rightSlope" in varsToSave:
            ...
            nc_rightSlope[:] = np.array(self.rightSlope, dtype="f4")#.T
        ...
        if (varsToSave == 'all' and saveAlsoNonDealiased) or "W_noDA" in varsToSave:
            ...
            nc_w_noDA.units = "m/s"
            nc_w_noDA[:] = np.array(self.W_noDA, dtype="f4")#.T
        ...
        if varsToSave == 'all' or "W" in varsToSave:
            ...
            nc_w.units = "m/s"
            nc_w[:] = np.array(self.W, dtype="f4")#.T
        ...
        if varsToSave == 'all' or "etaNoiseAve" in varsToSave:
            ...
            nc_noiseAve[:] = np.array(self.etaNoiseAve, dtype="f4")#.T
        ...
        if varsToSave == 'all' or "etaNoiseStd" in varsToSave:
            ...
            nc_noiseStd[:] = np.array(self.etaNoiseStd, dtype="f4")#.T
        ...
        if varsToSave == 'all' or "SNR" in varsToSave:
            ...
            nc_SNR.units = "dB"
            nc_SNR[:] = np.array(self.SNR, dtype="f4")#.T
...
class mrrRawData:
    ...
    def __init__(self, fname, debugStart=0, debugLimit=0, maskData=True, ncForm="NETCDF3_CLASSIC",removeOverlap=False,varsToSave='all'):
        """
        ...
        for f, file in enumerate(files):
            ...
            if removeOverlap == True: #Remove profiles corresponding to the day before and previous    

                data_day = [time.gmtime(dmrrkeys).tm_mday for dmrrkeys in dataMRR.keys()]

                data_day = np.array(data_day)    

                median_day = np.median(data_day)

                pixLimit = np.squeeze(np.where(data_day == median_day))

                debugStart = pixLimit[0]
                debugLimit = pixLimit[-1]
