...
"""
alternative pour gérer les execptions d'importations des fichiers numpy, netCDF4 : 
...
try:
    ...
    print(f"Erreur: Le module {missing_module} n'est pas installé.")
    print("Veuillez installer les modules nécessaires ainsi que la bonne version en utilisant le fichier requirements.txt.")
    print("Exécutez la commande suivante :")
...
"""
alternative pour gérer les execptions de mauvais path pour les fichier input et output:

import os

def check_file_path(file_path, file_type="input"):
    if not os.path.isfile(file_path):
        raise FileNotFoundError(f"Erreur : {file_type} '{file_path}' n'existe pas vérifier le path des fichiers contenant les .raw dans l'input de default_parameters.txt")

try:
	df = open('default_parameters.txt','r')
	df_lines = df.readlines()
	check_file_path(Input data path, file_type="entrée")
	check_file_path(Output data path, file_type="sortie")
	df.close()
