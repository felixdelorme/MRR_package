...
file1= 'C:/Users/rebouda/Documents/02-Data/MRR_nc_tests/OSUG-B_20141114.nc'
file1= 'C:/Users/rebouda/Documents/02-Data/MRR_nc_tests/OSUG-B_20140302.nc'
file1= 'C:/Users/rebouda/Documents/02-Data/MRR_Data/OSUG-B/MK_processed/202012/OSUG-B_202012.nc'
file1 = 'C:/Users/rebouda/Documents/02-Data/MRR_Data/OSUG-B/MK_processed/202103/OSUG-B_202103.nc'
file1 = 'C:/Users/rebouda/Documents/02-Data/MRR_Data/OSUG-B/MK_processed/202101/OSUG-B_202101.nc'
file1 = 'C:/Users/rebouda/Documents/02-Data/MRR_Data/OSUG-B/MK_processed/202004/OSUG-B_20200419.nc'

file1 = 'C:/Users/rebouda/Documents/02-Data/MRR_Data/OSUG-B/MK_processed/201705/OSUG-B_201705.nc' 

station_name='Aulnat'
alt_station = 330 #m.a.s.l : 230 m elevation of OSUG-B station; 330 m Clermont

#%% read satellite time file ML
file_sat_ML = 'C:/Users/rebouda/Documents/04-WorkResults/Alps_BB_GPM_DPR_rad50km_semi.csv'
list_time_sat = pd.read_csv(file_sat_ML, sep=';')
list_time_sat = pd.to_datetime(list_time_sat.Time_sat)
list_time_sat[0].strftime("%Y%m%d%H%M")

#%%
ML_DPR_vs_MRR = pd.DataFrame(columns=['heightBB_Ze','topBB_Ze','botBB_Ze','heightBB_W','topBB_W','botBB_W'])

for i in list_time_sat:
    sat_time=i.strftime("%Y-%m-%d %H:%M:00")
    file_MRR_time=i.strftime(station_name+"_%Y%m%d.nc")
    event_start = (i - dt.timedelta(hours=5)).strftime("%Y-%m-%d %H:%M:00")
    event_stop = (i + dt.timedelta(hours=5)).strftime("%Y-%m-%d %H:%M:00")
    
    print(file_MRR_time)
    # for path_file_MRR in glob.glob('C:/Users/rebouda/Documents/02-Data/MRR_Data/Aulnat/MK_processed/*/' + '*.nc'):
    #     print(files)
    path_file_MRR='C:/Users/rebouda/Documents/02-Data/MRR_Data/'+station_name+'/MK_processed/'+i.strftime("%Y%m/")+file_MRR_time
    if exists(path_file_MRR):
        ds=nc.Dataset(path_file_MRR, mode='r')
        time=ds.variables['time']
        
        if pd.to_datetime(event_start) < pd.to_datetime(time[:].data, unit='s')[0]:
            ind_start=0
        elif pd.to_datetime(event_start) < pd.to_datetime(time[:].data, unit='s')[-1]:
            ind_start=np.where(event_start == pd.to_datetime(time[:].data, unit='s'))[0][0]
        else :
            print('Time sat is Out of file data range')
            continue
        if pd.to_datetime(event_stop) > pd.to_datetime(time[:].data, unit='s')[-1]:
            ind_stop=time.size
        else :
...
#%
        ...
        date_event = pd.to_datetime(time[0], unit='s').strftime("%Y-%m-%d")
        ds.close()
        ...
        for i in range(0,len(Ze[:])):
            ...
            if ma.count_masked(Ze[i,:])/len(Ze[i,:]) > 0.3: #ma.count_masked(gradW[i,:])
        ...
        # plt.plot(W_filt[800,:],height[800,:],'+')
        # plt.plot(z_filtered[800,:],height[800,:],'--')
        Ze_filtered= ma.array(data=Ze_filtered,mask=Ze.mask, fill_value=-9999)
        
        #plt.plot(Ze_filtered[1500,:], height[1500,:], '+')
        ...
        plt.xlabel('Time [UTC]')
        plt.title('MRR OSUG-B ' +date_event+ ' \n Maximum reflectivity')
        plt.grid()
        ...
        hmaxZe_interpol=pd.DataFrame(hmaxZe).interpolate(method='linear', limit=90, 
                                                           limit_area='inside')
        ...
        hML_lim= hmaxZe_smooth.mean() + 600 #do not consider reflectivities 500m above the ML height when searching for the ML top
        ...
        hmingradZe = ma.masked_where(hmingradZe>(hmaxZe_smooth.mean()+600),hmingradZe) #restrict the search of MLtop to 500m above the MLheight
        ...
        #%
        hmingradZe_interpol=pd.DataFrame(hmingradZe).interpolate(method='linear', limit=90, 
                                                           limit_area='inside')
        hmingradZe_smooth = gaussian_filter1d(hmingradZe_interpol.values[:,0],sigma=10)
        ...
        #%
        hmaxgradZe_interpol=pd.DataFrame(hmaxgradZe).interpolate(method='linear', limit=90, 
                                                           limit_area='inside')
        ...
        hmaxgradZe_smooth = gaussian_filter1d(hmaxgradZe_interpol.values[:,0],sigma=15)
        ...
        plt.hist(W_filt)
        plt.plot(W_filt[1,:],height[1,:],'+')
        
        for i in range(0,len(W_filt[:])):
            #if more than 50% of the vertical bins are NaN --> reject all the values for the time step (i.e set Filter=False)
            if ma.count_masked(W_filt[i,:])/len(W_filt[i,:]) > 0.5: #ma.count_masked(gradW[i,:])
               #W_filt[i,:]= ma.masked_where(ma.count_masked(W_filt[i,:])/len(W_filt[i,:]) > 0.5,W_filt[i,:])
        ...
        #%
        h_interpol=pd.DataFrame(hmaxgrad).interpolate(method='linear', limit=90, 
                                                           limit_area='inside')
        ...
        # plt.hist(grad2W)
        # plt.plot(grad2W[400,:], height[400,:], '-+', label='grad2W')
        # plt.plot(gradW[400,:], height[400,:], '-+', label='grad1W')
        # plt.legend()
        ...
        #% convert the masked array to pd.df and interpolate the NAN
        hmaxgrad2W_interpol=pd.DataFrame(hmaxgrad2W).interpolate(method='linear', limit=90, 
                                                           limit_area='inside')
        hmaxgrad2W_smooth = gaussian_filter1d(hmaxgrad2W_interpol.values[:,0],sigma=20)
        ...
        #% convert the masked array to pd.df and interpolate the NAN
        hmingrad2W_interpol=pd.DataFrame(hmingrad2W).interpolate(method='linear', limit=90, 
                                                           limit_area='inside')
        hmingrad2W_smooth = gaussian_filter1d(hmingrad2W_interpol.values[:,0],sigma=20)
        ...
        ML_DPR_vs_MRR=ML_DPR_vs_MRR.append(BB_MRR.to_frame(name=sat_time).T)

    else:
        print('No files matching the times in the folder')
