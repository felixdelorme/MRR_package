...
file1= '/Users/felixdelorme/ESISAR/Stage/stage_felix_2024_MRR_Bolivie/MK_packaged_v2/MK_processed/PYRAMIDE/202405/0512_60s_PLATAFORMA.nc'
path_save = '/Users/felixdelorme/ESISAR/Stage/stage_felix_2024_MRR_Bolivie/3 Images/PLATAFORMA MRR'

#%%
ds=nc.Dataset(file1, mode='r')
print(ds.variables.keys())

#subset the data to event
sat_time = '2024-05-12 09:00:00'
event_start = '2024-05-12 07:00:00'
event_stop = '2024-05-12 10:00:00'
event_date = '2024-05-12'


alt_station = 230 #m.a.s.l : 230 masl elevation of OSUG-B station
...
for i in range(0,len(Ze[:])):
    ...
    if ma.count_masked(Ze[i,:])/len(Ze[i,:]) > 0.2: #ma.count_masked(gradW[i,:])
...
hmaxZe_interpol=pd.DataFrame(hmaxZe).interpolate(method='linear', limit=90, 
                                                   limit_area='inside')
hmaxZe_smooth = gaussian_filter1d(hmaxZe_interpol.values[:,0],sigma=10)
...
hmingradZe_interpol=pd.DataFrame(hmingradZe).interpolate(method='linear', limit=90, 
                                                   limit_area='inside')
hmingradZe_smooth = gaussian_filter1d(hmingradZe_interpol.values[:,0],sigma=5)
...
hmaxgradZe_smooth = gaussian_filter1d(hmaxgradZe_interpol.values[:,0],sigma=10)
...
#%%plot of profile at sepecific time
...
fig.tight_layout()
#plt.savefig(path_save+'Ze_profile_MRR.png',format='png',dpi=300)
plt.show()
...
h_interpol=pd.DataFrame(hmaxgrad).interpolate(method='linear', limit=90, 
                                                   limit_area='inside')
h_smooth = gaussian_filter1d(h_interpol.values[:,0],sigma=5)
...
hmaxgrad2W_interpol=pd.DataFrame(hmaxgrad2W).interpolate(method='linear', limit=90, 
                                                   limit_area='inside')
hmaxgrad2W_smooth = gaussian_filter1d(hmaxgrad2W_interpol.values[:,0],sigma=10)
...
hmingrad2W_interpol=pd.DataFrame(hmingrad2W).interpolate(method='linear', limit=90, 
                                                   limit_area='inside')
hmingrad2W_smooth = gaussian_filter1d(hmingrad2W_interpol.values[:,0],sigma=10)
...
ax1.grid()
fig.tight_layout()
plt.savefig(path_save+'W_profile_MRR.png',format='png',dpi=300)
plt.show()
